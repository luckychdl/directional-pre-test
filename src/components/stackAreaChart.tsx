...
function toPercentRows<T extends { week: string }>(
  ...
  return rows.map((row) => {
    ...
    keys.forEach((k) => {
      ...
      next[String(k)] = sum === 0 ? 0 : +((v / sum) * 100).toFixed(1);
    });
  ...
  return (
    <div className={styles.bar_chart}>
      ...
      <div className={styles.chart}>
        <h3>Weekly Mood Trend (Stacked Area %)</h3>
        <EChartsReact
          option={moodOption}
      ...
      <div className={styles.chart}>
        <h3>Weekly Workout Trend (Stacked Area %)</h3>
        <EChartsReact
